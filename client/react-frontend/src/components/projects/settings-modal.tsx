...
import { Palette, Bell, Shield, Globe, Monitor, Smartphone } from "lucide-react"
    ...
    <div className="space-y-6">
          ...
          {themes.map((themeOption) => (
            ...
            >
              <div className="flex items-center space-x-2 mb-2">
                <div className="w-4 h-4 rounded-full" style={{ backgroundColor: themeOption.colors[0] }} />
                <div className="w-4 h-4 rounded-full" style={{ backgroundColor: themeOption.colors[1] }} />
              </div>
              <span className="text-sm font-medium">{themeOption.name}</span>
              {theme === themeOption.id && <Badge className="absolute -top-2 -right-2 bg-blue-500">Activo</Badge>}
            </button>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex items-center justify-between p-4 border rounded-lg">
          <div className="flex items-center space-x-3">
            {isDark ? <Monitor className="w-5 h-5" /> : <Smartphone className="w-5 h-5" />}
            <div>
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <Slider defaultValue={[16]} max={24} min={12} step={2} className="w-full" />
        </div>
      </div>
    </div>
  )
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          {[
            { label: "Nuevas tareas asignadas", desc: "Recibir notificación cuando te asignen una tarea" },
            { label: "Cambios en subtareas", desc: "Notificar cuando cambien estados de subtareas" },
            { label: "Menciones en comentarios", desc: "Cuando alguien te mencione en un comentario" },
            { label: "Recordatorios de vencimiento", desc: "Alertas antes de que venzan las tareas" },
          ].map((item, index) => (
      ...
      <div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Tono de notificación</label>
  ...
  const renderPrivacySettings = () => (
    <div className="space-y-6">
      <div>
        ...
        <div className="space-y-4">
          {[
            { label: "Mostrar estado en línea", desc: "Otros usuarios pueden ver cuando estás activo" },
            { label: "Mostrar última actividad", desc: "Mostrar cuándo fue tu última actividad" },
            { label: "Permitir menciones", desc: "Otros pueden mencionarte en comentarios" },
          ].map((item, index) => (
            <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                <p className="font-medium">{item.label}</p>
  ...
  const renderGeneralSettings = () => (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select defaultValue="america/havana">
              ...
              <SelectContent>
                <SelectItem value="america/havana">América/La Habana</SelectItem>
      ...
      <div>
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">Horas de trabajo por día</label>
          ...
          <div className="flex items-center justify-between p-3 border rounded-lg">
            <div>
              ...
              <p className="text-sm text-gray-500">Silenciar notificaciones durante trabajo</p>
            </div>
            <Switch />
          </div>
        </div>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] p-0">
        ...
        <div className="hidden md:flex h-[600px]">
          ...
          {/* Main Content */}
          <div className="flex-1 p-6 overflow-y-auto">{renderCategoryContent()}</div>
        </div>
                ...
                <Collapsible key={category.id}>
                  ...
                  <CollapsibleContent className="mt-2 p-4 border rounded-lg bg-gray-50 dark:bg-gray-800/50">
                    <div onClick={() => setActiveCategory(category.id)}>
                      {category.id === "appearance" && renderAppearanceSettings()}
                      {category.id === "notifications" && renderNotificationSettings()}
        ...
        <div className="border-t p-4 flex justify-between items-center">
          <p className="text-sm text-gray-500">SYSGD v2.1.0 - Sistema de Gestión de Documentos</p>
